    ...
    pub fn do_pick(&mut self, pick: Pick) {
        match pick {
            Pick::Nothing => {
                ...
                self.event_loop_proxy.send_event(LabEvent::ControlState(ControlState::Viewing)).unwrap()
            }
            Pick::Joint(joint_index) => {
                self.camera.set_target(AroundJoint(joint_index));
                self.event_loop_proxy.send_event(LabEvent::ControlState(ControlState::ShowingJoint(joint_index))).unwrap();
            }
            Pick::Interval { joint, id, interval } => {
                ...
                self.event_loop_proxy.send_event(LabEvent::ControlState(ControlState::ShowingInterval(interval_details))).unwrap();
            }
        }
