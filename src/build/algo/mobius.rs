...
pub fn generate_mobius(segments: usize) -> Fabric {
    ...
    for joint_index in 0..joint_count {
        ...
        mf.fabric
            .create_joint(location(joint_index % 2 == 0, angle));
    }
    for joint_index in 0..joint_count {
        let joint = |offset: usize| (joint_index * 2 + offset) % joint_count;
        mf.fabric.create_interval(joint(0), joint(2), 0.4, Role::Pulling);
        mf.fabric.create_interval(joint(0), joint(1), 1.0, Role::Pulling);
        mf.fabric.create_interval(joint(0), joint(3), 3.0, Role::Pulling);
