...
pub fn export(brick_name: BrickName, baked_code: &str) {
    let source =
        std::fs::read_to_string(BAKED_BRICKS_PATH).expect("Failed to read baked_bricks.rs");
    ...
    std::fs::write(BAKED_BRICKS_PATH, &new_source).expect("Failed to write baked_bricks.rs");
}
...
fn substitute_baked_section(source: &str, func_name: &str, replacement: &str) -> Option<String> {
    ...
    let line_start = source[..scale_start]
        .rfind('\n')
        .map(|i| i + 1)
        .unwrap_or(0);
