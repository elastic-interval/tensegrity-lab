...
fn test_brick_library_construction() {
    ...
    let brick_names = [
        BrickName::SingleLeftBrick, BrickName::SingleLeftBrick, BrickName::OmniBrick,
        BrickName::TorqueBrick, BrickName::TorqueRightBrick, BrickName::TorqueLeftBrick, BrickName::EqualsBrick
    ];
...
fn test_individual_bricks() {
    ...
    assert!(single_r.proto.alias.0.contains(&FaceTag::Brick(BrickName::SingleLeftBrick)));
    ...
    assert!(single_l.proto.alias.0.contains(&FaceTag::Brick(BrickName::SingleLeftBrick)));
    assert!(single_l.baked.is_some());

    let omni_brick = omni();
    assert!(omni_brick.proto.alias.0.contains(&FaceTag::Brick(BrickName::OmniBrick)));
    assert_eq!(omni_brick.proto.pushes.len(), 6);
    assert!(omni_brick.baked.is_some());

    let torque_brick = torque();
    assert!(torque_brick.proto.alias.0.contains(&FaceTag::Brick(BrickName::TorqueBrick)));
    assert_eq!(torque_brick.proto.pushes.len(), 9);
    assert!(torque_brick.baked.is_some());

    let torque_r = torque_right();
    assert!(torque_r.proto.alias.0.contains(&FaceTag::Brick(BrickName::TorqueRightBrick)));
    assert!(torque_r.baked.is_some());

    let torque_l = torque_left();
    assert!(torque_l.proto.alias.0.contains(&FaceTag::Brick(BrickName::TorqueLeftBrick)));
    assert!(torque_l.baked.is_some());

    let equals_brick = equals();
    assert!(equals_brick.proto.alias.0.contains(&FaceTag::Brick(BrickName::EqualsBrick)));
